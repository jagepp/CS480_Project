...
public class CharacterServletUpdate extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CharacterServletUpdate() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PCharacter form = new PCharacter();
		...
		form.setpClass(info.get(1));
		form.setLightLevel(Integer.valueOf(info.get(2)));
		form.setSubclass(info.get(3));
		form.setId(Integer.valueOf(info.get(4)));
		
		try {
			characterservice.updateCharacter(form);
